...
export type GoogleBuyerReservation = { id:string; status:string; start:string; end:string; revision:number; desiredStart:string|null; desiredEnd:string|null; recoveryCode?:string|null };
...
export async function readGoogleBuyerReservation(id:string,buyer:string):Promise<GoogleBuyerReservation|null> {
  const result=await db.from("google_booking_reservations_v1").select("id,status,starts_at,ends_at,revision,desired_starts_at,desired_ends_at,recovery_code").eq("id",id).eq("buyer_id",buyer).maybeSingle();
  if(result.error)throw new Error("Could not check your booking");
  const row=result.data;
  return row?{id:row.id,status:row.status,start:row.starts_at,end:row.ends_at,revision:Number(row.revision),desiredStart:row.desired_starts_at??null,desiredEnd:row.desired_ends_at??null,recoveryCode:row.recovery_code??null}:null;
}
