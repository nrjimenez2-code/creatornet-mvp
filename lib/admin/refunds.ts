...
export async function processRefundOperation(
  ...
  try {
    // Serialize future monthly/installment admission before any customer/fee refund,
    // including retries of a previously created Stripe refund. An already
    // dispatched payment must be reconciled first; an expired lease isn't proof
    // of failure. The durable hold survives this operation's retry/failure.
    const admission = await store.coordinateInstallmentRefund(operation.id, token);
    if (admission === "reconciliation_required") {
      operation = await markNeedsReconciliation(store, operation, token,
        "A payment needs reconciliation before this refund can continue.",
        { installment_collection: "held", installment_payment: "reconciliation_required" });
