...
3. Use a distinct QA experiment to test both arms, session persistence, zero-event viewers and purchase/refund updates. Do not include QA accounts in a real pilot population. At present enrollment is all authenticated Discover users while the flag is set; therefore real activation needs a deployment/population separated from QA, or an explicit eligibility implementation first.
