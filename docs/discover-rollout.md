...
3. Extend the completed staging migration and authenticated onboarding/feed checks to the remaining commercial workflows. Existing payment-ledger triggers and permissions were reviewed before staging application; focused local fixtures do not substitute for all deployed workflow checks.
4. Verify metadata coverage for existing videos after backfill. The shared media worker update is deployed and the existing processed test video returns a verified duration. External and unprocessed sources keep qualified-view tracking without a completion award.
...
The application is deployed on the staging Preview branch with the feature enabled. Production migration, scheduling-account configuration, and commercial-weight calibration remain incomplete.
...
## Deployed acceptance update (2026-09-13 UTC)

Staging Preview commit 312266194319a1cf0c5b11161ca8676ffe7d4a9e built successfully (Vercel EbXsJY42gkb8rdwR4v1o2VHx3wvB). The branch-scoped flag enables Discover v4. Authenticated onboarding saved two canonical interests and generated a matching session. Playback recorded qualified views; the browser-discovered feedback-button wiring bug was fixed and dismissal saved one negative event. Following's twelve-post snapshot was verified newest-first. The disposable account, profile, follow, sessions and test events were removed; original staging profile/post counts remain 3/13.

Shared Cloudflare worker version 1bf3bade-329c-453f-a48a-b6d27a2c51c2 was deployed with explicit approval using a content-only update. Bindings/settings were verified unchanged. Previous version 540761f6-a34b-45fe-8a5a-ddb41e3e95f1 remains the rollback reference. The existing processed test video returned two seconds and retained its normal playback redirect. Nine worker tests and fourteen media tests passed; TypeScript checking passed. The trusted original production R2 hostname is accepted for duration lookup, while credentials, fragments and unrelated domains are rejected.

Production media backfill completed: the three eligible source videos now publish ETag-bound trusted durations of 39.7, 28.7 and 48.4 seconds. Each source was submitted once to the existing queue. All three Stream jobs and downloadable copies reached Ready. Each /auto/ playback URL returned a 302 followed by a 200 video/mp4 response. Original sources were retained. This covers the current three-video production inventory; separate staging/external media remains outside this worker's bucket.
