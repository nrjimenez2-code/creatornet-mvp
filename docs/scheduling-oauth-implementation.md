...
Current state (2026-09-14 UTC): [the production release guide](discover-production-release.md) is authoritative for deployment and acceptance. All three providers have genuine staging create/reschedule/cancel evidence. Cal.com and Google normal disconnect/reconnect passed; Cal.com natural expired-token renewal also passed, with one manual status retry. Provider-side revocation and other expiry cases are not all verified live. Candidate 27fae59 passed full CI and Preview deployment. Calendar privacy is published through production PR #170; feature PR #169 remains draft. Google public verification, Cal.com production approval, final review and feature rollout remain incomplete. The sections below are historical implementation checkpoints, not current missing-feature lists.
...
An earlier staging upload mismatch described in the checkpoints below preceded the subsequently completed live booking and purchase tests. Do not treat that historical draft/upload checkpoint as current release evidence.
...
## Legacy ranking permission review and recovered browser — 2026-09-14 UTC

Staging exposed three SECURITY DEFINER metric mutators to PUBLIC/anon/authenticated despite server-only application callers: bump_interest_score, bump_post_metrics and bump_post_metrics_scored. Migration 20260914002844_restrict_legacy_ranking_mutations.sql revokes browser execution and explicitly retains service_role access, while tolerating installations without the optional legacy functions. Applied staging connector version: 20260914003035. Fourteen focused tests (including real PostgreSQL role checks and nested score updates) and TypeScript passed. Staging readback confirmed all three browser grants false/service grants true; real service-role calls completed inside a rolled-back transaction. Production is unchanged. Other advisor findings still require review.

User restarted Edge extension and browser reads now work. Calendly My Apps was verified empty. Prepared unsaved CreatorNet Staging Web/Sandbox registration at https://developer.calendly.com/console/apps/new with staging /api/scheduling/oauth/callback/calendly redirect and users:read event_types:read scheduled_events:read webhooks:read webhooks:write. User approval is pending for final Create because credentials are generated; do not assume submitted. Calendly tab 2112068674 and Cal.com tab 2112068704 marked handoff. Cal.com currently shows Creator Test onboarding, username blank. No OAuth credentials have been read or stored.

## Google reconnect retry wakeup — 2026-09-14 UTC

New local migration 20260914003852_google_reconnect_retry_wakeup.sql makes current-revision retry jobs immediately due when a Google connection transitions back to connected. Attempts, mutation evidence, active processing leases, completed/stale revisions and ordinary connected-status updates retain their existing state/backoff. Twenty-seven Google database tests and TypeScript passed. Applied staging version 20260914004014; installed trigger and service-only function privileges verified. Production unchanged.

Preview deployment for pushed e7d579d succeeded: creatornet-mvp/9mZEzdsv6ScNyAoRJNkq8tnFxR6b. This newest retry-wakeup migration/test change is a later local commit until pushed. Calendly app exists; approved credential transfer still awaits user filling the two masked secrets in the prepared Vercel form. Keep both handoff tabs open and do not repeat the granted transfer approval. Real OAuth/booking/payment acceptance, remaining recovery review, performance/pilot and production rollout remain incomplete.
