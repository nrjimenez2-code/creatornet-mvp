...
import { schedulingAvailable, schedulingOrigin } from "@/lib/schedulingConfig";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const providers = ["calcom", "calendly"] as const;
    if (process.env.SCHEDULING_OAUTH_ENABLED !== "true") return NextResponse.json({ connections: providers.map(provider => ({
    ...
    for (const row of rows ?? []) {
      if (isSchedulingProvider(row.provider) && schedulingAvailable(row.provider) &&
          ["connected", "error", "pending"].includes(row.status) &&
          (!row.last_checked_at || Date.parse(row.last_checked_at) < Date.now() - 300_000)) {
        try { await refreshSchedulingConnection(user.id, row.provider); }
        catch {
    ...
    for (const provider of providers) {
      ...
      connections.push({ provider, available: schedulingAvailable(provider), status: row?.status ?? "disconnected", accountName: row?.account_name ?? null, eventTypes });
    }
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    await disconnectSchedulingConnection(user.id, provider);
