...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await executeAdminRefundAction({ admin: context.admin, actorId: context.user.id }, { kind: "create", input });
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const result = await executeAdminRefundAction({ admin: context.admin, actorId: context.user.id }, { kind: "retry", operationId: refundId });
    return responseForResult(result);
  } catch (error) {
    if (error instanceof RefundWorkflowError) {
      return refundWorkflowErrorResponse(error);
