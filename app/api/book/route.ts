...
// Never expose this in NEXT_PUBLIC_*
...
export async function GET(req: Request) {
  try {
    ...
    // 1) If post_id is present, prefer the post's explicit booking_url
    if (postId) {
    ...
    // 2) Closers table (weighted round-robin; here we just pick highest weight)
    ...
    if (!closerErr && closerRows && closerRows.length > 0) {
    ...
    // 3) profiles.booking_url as a final fallback (if you keep one there)
    ...
    return NextResponse.json({ error: "Booking router failed" }, { status: 500 });
  }
}
