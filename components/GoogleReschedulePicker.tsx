...
export default function GoogleReschedulePicker({id,revision,token,onChanged,onClose}:{id:string;revision:number;token?:string;onChanged:(reservation:Reservation)=>void;onClose:()=>void}) {
 ...
 return <section aria-label="Choose a new booking time" className="space-y-3 rounded-lg border border-white/20 p-4">
  ...
  {loading?<p role="status">Checking available times…</p>:<fieldset disabled={busy} className="space-y-2"><legend>Available times</legend>{slots.map(slot=><label className="flex gap-3 p-2" key={slot.start}><input type="radio" name="new-time" checked={selected===slot.start} onChange={()=>setSelected(slot.start)}/>{format(slot.start)}</label>)}{!slots.length&&!error&&<p>No available times this week.</p>}</fieldset>}
