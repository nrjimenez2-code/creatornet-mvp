  ...
  return <main className={styles.page}>
    ...
    <div className={styles.content}>
      ...
      <div className={styles.layout}>
        <nav aria-label="Analytics sections"><label className={styles.srOnly} htmlFor="analytics-section">Analytics section</label>
          <select id="analytics-section" value={detail.section} disabled={pending} onChange={e => navigate(e.target.value, String(window.days))}>{analyticsSections.map(section => <option key={section} value={section}>{sectionLabels[section]}</option>)}</select>
        </nav>
        <section className={styles.detail} aria-label={`${detail.title} analytics`} aria-busy={pending}>
          ...
          <div className={styles.summary}><h2>{detail.title}</h2><div className={styles.total}>{detail.unavailable ? '—' : format(total)}</div><p>{detail.metric}</p></div>
          {detail.unavailable ? <div className={styles.error} role="alert"><h3>Analytics unavailable</h3><p>We couldn’t load this section. Try again or choose a shorter date range.</p><button onClick={() => startTransition(() => router.refresh())} disabled={pending}>Try again</button></div> : <>
            ...
            <dl className={styles.stats}><div><dt>Total</dt><dd>{format(total)}</dd></div><div><dt>Daily average</dt><dd>{format(total / window.days)}</dd></div><div><dt>Peak day</dt><dd>{total && peak ? dayLabel(peak.date) : '—'}</dd></div>{detail.stats.map(stat => <div key={stat.label}><dt>{stat.label}</dt><dd>{stat.value}</dd></div>)}</dl>
