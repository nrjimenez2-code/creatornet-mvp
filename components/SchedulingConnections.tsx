...
export default function SchedulingConnections({ purpose, value, onSelect }: Props) {
  ...
  function connect(provider: BookingProvider) {
    // The entire composer, including File objects, remains mounted in this window.
    const popup = window.open(`/scheduling/connect?provider=${provider}`, "_blank", "popup,width=600,height=760");
    if (!popup) {
  ...
  return <section aria-label="Booking provider connections" className="space-y-3 rounded-xl border border-white/15 bg-white/5 p-4 text-white">
    ...
    {items?.map(item => {
      ...
      return <div key={item.provider} className="space-y-2 rounded-lg border border-white/10 p-3">
        <p className="font-medium">{name} {connected ? "connected" : reconnect ? "needs reconnection" : item.status === "disconnecting" ? "disconnecting" : "not connected"}</p>
        ...
        {item.available && !connected && item.status !== "disconnecting" && <button type="button" className="rounded-lg bg-white px-3 py-2 text-sm text-black" onClick={() => connect(item.provider)}>{reconnect ? "Reconnect" : "Connect"} {name}</button>}
        ...
        {connected && onSelect && <label className="block text-sm">{purpose === "session" ? "Session" : "Sales call"} event
