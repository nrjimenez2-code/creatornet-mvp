...
test.each(["link_sent","completed"])("exact %s link shows the final cent and labels saved fees as first-payment estimates",async(status)=>{
  const fetchMock=await renderPage();
  fetchMock.mockResolvedValueOnce(response({payment:{id:"exact-payment",plan_type:"installment",status,
    installment_months:3,installment_amount_cents:66633,amount_total_cents:199900,creator_net_cents:56675,
    platform_fee_cents:7996,processing_fee_cents:1962,total_creator_deduction_cents:9958,
    fee_schedule_version:"synthetic-card",installment_collection_version:"exact-cents-held-v1",
    currency:"usd",created_at:"2026-09-06T12:00:00Z"}}));
  await click(button("Generate installment link"));await submit();
  expect(container.textContent).toContain("3 payments");
  expect(container.textContent).toContain("$666.34 final payment");
  expect(container.textContent).toContain("$1,999.00 total");
  expect(container.textContent).toContain("First payment fee estimate");
  expect(container.textContent).not.toMatch(/Expected per payment|Recorded per payment/);
  expect(container.textContent).toContain("not proof of payment");
});
