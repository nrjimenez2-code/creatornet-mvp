...
function fakeStore(initial: RefundOperation) {
  ...
  const store: RefundOperationStore = {
    ...
    coordinateInstallmentRefund: jest.fn(async () => "not_applicable"),
...
describe("durable refund processing", () => {
  test.each([null, "re_existing"])("unresolved installment blocks all Stripe refund/fee calls, existing refund %s", async (stripeRefundId) => {
    const state = fakeStore(operation({ stripeRefundId }));
    jest.mocked(state.store.coordinateInstallmentRefund).mockResolvedValue("reconciliation_required");
    const stripe = fakeStripe();
    const result = await processRefundOperation(state.store, stripe as never, state.current().id);
    expect(result.disposition).toBe("needs_reconciliation");
    expect(result.operation.reconciliationInfo).toEqual({ installment_collection: "held", installment_payment: "reconciliation_required" });
    expect(stripe.refunds.create).not.toHaveBeenCalled();
    expect(stripe.refunds.retrieve).not.toHaveBeenCalled();
    expect(stripe.applicationFees.retrieve).not.toHaveBeenCalled();
    expect(stripe.applicationFees.createRefund).not.toHaveBeenCalled();
    expect(state.store.updateClaimedOperation).toHaveBeenCalledWith(state.current().id, expect.any(String),
      expect.objectContaining({ processing_token: null }));
  });
  test("coordination failure cannot fall through to Stripe", async () => {
    const state = fakeStore(operation());
    jest.mocked(state.store.coordinateInstallmentRefund).mockRejectedValue(new Error("Coordination unavailable"));
    const stripe = fakeStripe();
    expect((await processRefundOperation(state.store, stripe as never, state.current().id)).disposition).toBe("needs_reconciliation");
    expect(stripe.refunds.create).not.toHaveBeenCalled();
    expect(stripe.applicationFees.createRefund).not.toHaveBeenCalled();
  });
  test("a held refund proceeds only on a later admitted retry with its original idempotency key", async () => {
    const state = fakeStore(operation());
    jest.mocked(state.store.coordinateInstallmentRefund).mockResolvedValueOnce("reconciliation_required").mockResolvedValue("held");
    const stripe = fakeStripe();
    await processRefundOperation(state.store, stripe as never, state.current().id);
    const result = await processRefundOperation(state.store, stripe as never, state.current().id);
    expect(result.disposition).toBe("completed");
    expect(stripe.refunds.create).toHaveBeenCalledTimes(1);
    expect(state.store.coordinateInstallmentRefund).toHaveBeenCalledTimes(2);
    expect(jest.mocked(state.store.coordinateInstallmentRefund).mock.invocationCallOrder[1]).toBeLessThan(stripe.refunds.create.mock.invocationCallOrder[0]);
  });
