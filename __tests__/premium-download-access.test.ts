...
async function download(postId = "post-one") {
  const { GET } = await import("@/app/api/watch/[postId]/route");
  return GET(new NextRequest("https://example.invalid/api/watch/post-one?buyer_id=buyer-one"), { params: Promise.resolve({ postId }) });
}

test("owner deletes a paid post; buyer still downloads, while a nonbuyer cannot", async () => {
  const id = "11111111-1111-4111-8111-111111111111";
  const post = { id, creator_id: "creator-one", premium_path: premiumPath, removed_at: null as string | null };
  db = createMockClient(op => {
    if (op.table === "posts" && op.kind === "update") {
      if (op.filters.creator_id !== post.creator_id || op.filters.id !== id) return { data: null, error: null };
      Object.assign(post, op.payload); return { data: { id }, error: null };
    }
    if (op.table === "posts" && op.kind === "select") {
      // Respect discovery filters: an accidental filter in delivery must fail this test.
      if (op.isFilters.some(filter => filter.column === "removed_at") && post.removed_at) return { data: null, error: null };
  ...
  const response = await DELETE(new NextRequest(`https://example.invalid/api/posts/${id}`, { method: "DELETE" }), { params: Promise.resolve({ postId: id }) });
  expect(response.status).toBe(200); expect(post.removed_at).toEqual(expect.any(String));
  user = { id: "buyer-one" }; expect((await download(id)).status).toBe(200);
  expect(sign).toHaveBeenCalledWith(premiumPath, 3600);
  sign.mockClear(); user = { id: "nonbuyer" }; expect((await download(id)).status).toBe(402);
  expect(sign).not.toHaveBeenCalled();
  expect(db.ops.filter(op => op.kind !== "select")).toHaveLength(1);
});
